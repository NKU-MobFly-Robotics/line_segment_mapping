...
private:
  ...
  void publishLineSegmentMapVisualization(visualization_msgs::Marker& line_segment_map_marker);
...
SlamKarto::SlamKarto() : got_map_(false), laser_count_(0), transform_thread_(NULL)
{
  ...
  line_segment_map_marker_publisher_ = node_.advertise<visualization_msgs::Marker>("line_markers", 1);
  ...
  if (private_nh_.getParam("scan_buffer_maximum_scan_distance", scan_buffer_maximum_scan_distance))
    mapper_ptr_->setParamScanBufferMaximumScanDistance(scan_buffer_maximum_scan_distance);
  ...
  if (private_nh_.getParam("loop_match_maximum_variance_coarse", loop_match_maximum_variance_coarse))
    mapper_ptr_->setParamLoopMatchMaximumVarianceCoarse(loop_match_maximum_variance_coarse);
  ...
  if (private_nh_.getParam("loop_match_minimum_response_coarse", loop_match_minimum_response_coarse))
    mapper_ptr_->setParamLoopMatchMinimumResponseCoarse(loop_match_minimum_response_coarse);
  ...
  if (private_nh_.getParam("loop_match_minimum_response_fine", loop_match_minimum_response_fine))
    mapper_ptr_->setParamLoopMatchMinimumResponseFine(loop_match_minimum_response_fine);
  ...
  if (private_nh_.getParam("correlation_search_space_dimension", correlation_search_space_dimension))
    mapper_ptr_->setParamCorrelationSearchSpaceDimension(correlation_search_space_dimension);
  ...
  if (private_nh_.getParam("correlation_search_space_resolution", correlation_search_space_resolution))
    mapper_ptr_->setParamCorrelationSearchSpaceResolution(correlation_search_space_resolution);
  ...
  if (private_nh_.getParam("correlation_search_space_smear_deviation", correlation_search_space_smear_deviation))
    mapper_ptr_->setParamCorrelationSearchSpaceSmearDeviation(correlation_search_space_smear_deviation);
  ...
  if (private_nh_.getParam("loop_search_space_dimension", loop_search_space_dimension))
    mapper_ptr_->setParamLoopSearchSpaceDimension(loop_search_space_dimension);

  double loop_search_space_resolution;
  if (private_nh_.getParam("loop_search_space_resolution", loop_search_space_resolution))
    mapper_ptr_->setParamLoopSearchSpaceResolution(loop_search_space_resolution);
  ...
  if (private_nh_.getParam("loop_search_space_smear_deviation", loop_search_space_smear_deviation))
    mapper_ptr_->setParamLoopSearchSpaceSmearDeviation(loop_search_space_smear_deviation);
...
void SlamKarto::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  {
    ROS_DEBUG("added scan at pose: %.6f %.6f %.6f", odom_pose.GetX(), odom_pose.GetY(), odom_pose.GetHeading());

    // visualization_msgs::MarkerArray marray;
    // solver_->publishGraphVisualization(marray);
    // marker_publisher_.publish(marray);

    visualization_msgs::Marker lineSegmentMapMarker;
    publishLineSegmentMapVisualization(lineSegmentMapMarker);
    line_segment_map_marker_publisher_.publish(lineSegmentMapMarker);
...
bool SlamKarto::updateMap()
{
  ...
  karto::OccupancyGrid* occ_grid =
      karto::OccupancyGrid::CreateFromScans(mapper_ptr_->GetAllProcessedScans(), resolution_);
  ...
  if ((processed = mapper_ptr_->Process(range_scan, line_segments)))
  {
    ...
    try
    {
      tf_.transformPose(odom_frame_,
                        tf::Stamped<tf::Pose>(corrected_pose_tf.inverse(), scan->header.stamp, base_frame_),
                        odom_to_map);
    }
    catch (tf::TransformException e)
    {
